...
**M3 — Code Mode (branch `m3-code-mode`, committed, NOT yet merged): code-complete + verified locally, prod deploy BLOCKED on plan.**
...
- **Verified end-to-end under `wrangler dev`** (which runs a real workerd Worker Loader): one `codemode` tool call, one JS snippet against `training.*`, calls dispatched back to the DO over RPC. Read (`getProgram`), write (`logSet`), and mutate (`adjustProgram` deload) all round-trip through the sandbox and **persist to DO state** (read-back confirmed 125→112). Injury/context awareness carries through — on the money prompt the coach refused to auto-bump a suspicious 225 for a detrained, ankle-injured lifter.
- **Blocker (prod only):** `wrangler deploy` fails with API error **10195 — "you must switch to a paid plan"**. Dynamic Worker Loader gates on **Workers Paid**; this account is still on **Free**. M0–M2 shipped on Free because Workers + DO + SQLite don't require Paid — Code Mode is the first feature that does. **The PLAN's Phase 0 note that Paid was provisioned was inaccurate** — the upgrade never actually landed. The failed deploy was rejected at version-upload, so **prod is untouched (still M2)**. Unblock = upgrade at dash → Workers & Pages → Plans (~$5/mo), then re-run `wrangler deploy`.
...
6. **Code Mode SDK drift, again.** `@cloudflare/codemode` is now **0.4.2** (PLAN snapshot said 0.1.0, Feb 2026). The `DynamicWorkerExecutor` + `createCodeTool({ tools, executor })` shape from the plan still holds; the package also grew a heavier `createCodemodeRuntime` path (durable executions, approvals, snippets) we didn't need. Auto type-gen handles our `jsonSchema()` tools (it detects the AI SDK wrapper), so no hand-written `types` needed.
7. **`$schema` rejection returns via the Code Mode tool.** M2's four tools dodge Heroku's `$schema` rejection by hand-writing JSON Schema, but `createCodeTool`'s generated `codemode` tool builds its own StandardSchema that re-emits a top-level `$schema`. Fix: the `model.ts` request shim now strips `$schema` **recursively** from the outgoing body (pure JSON-Schema dialect metadata, always safe to drop) — one place, covers any future tool.
8. **Dynamic Worker Loader gates on Workers Paid — and free-tier deploys hide it.** `wrangler dev` runs the Worker Loader locally with no plan check, so Code Mode looked fully done before deploy. Only `wrangler deploy` surfaced error 10195: the account is on Free. Lesson: a binding that works in `wrangler dev` is not proof it's provisioned in prod — verify the plan/entitlement early, exactly as Phase 0 step 1 intended (that step was marked done but wasn't).

**Current prod state:** still **M2** (front squat opener 125, pristine seed). The M3 deploy was rejected on the plan gate, so the live version is unchanged. `/reseed` disabled in prod (no `RESEED_TOKEN` secret).

**Next: unblock M3 deploy.** Code is done on `m3-code-mode` and proven locally. One action needed (billing, user-only): upgrade the account to **Workers Paid** at dash → Workers & Pages → Plans, then `npx wrangler deploy` and run the money prompt in prod. After that: merge `m3-code-mode` → `main`, then M4 (live WS session) / M5 (polish + `FRICTION.md`).
